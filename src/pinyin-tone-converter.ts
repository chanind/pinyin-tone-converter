/*
Based on PinyinConverter by David Chanin and Jen Liu @quizlet 2013
...
const getReplacement = (match: string) => {
  ...
  const tone = match.slice(-1).toString() as tone;
  const word = match
    .slice(0, -1)
    .replace('v', 'ü')
    .replace('V', 'Ü');
  for (const [base, vowel] of Object.entries(accentMap)) {
    if (word.indexOf(base) >= 0) {
      const vowelChar = (vowel.match(/.\*/) as RegExpMatchArray)[0] as vowel;
      const vowelNum = vowels[vowelChar];
      const accentedVowelChar = pinyin[tone][vowelNum];
      const replacedWord = word.replace(base, vowel).replace(vowelChar, accentedVowelChar);
      return replacedWord;
    }
  }
  return match;
...
const getAccentMap = (): AccentMap => {
  if (!accentMap) {
    ...
    const populatedAccentMap: AccentMap = {};
    nostars.split(' ').forEach((base, i) => {
      populatedAccentMap[base] = starsArray[i];
    });
    accentMap = populatedAccentMap;
  }
  return accentMap;
};

export const convertPinyinTones = (text: string) => {
  let convertedText = text;

  // Find words with a number behind them, and replace with callback fn.
  const matches = text.match(pinyinRegex);
  if (!matches) return convertedText;
